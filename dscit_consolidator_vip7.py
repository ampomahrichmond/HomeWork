...
.tbl th{background:#F0F6F0;color:var(--td-xd);text-align:center;
...
.tbl th:hover{background:#E4EFE4}
.tbl td{padding:7px 9px;border-bottom:1px solid #EDF3ED;
 vertical-align:middle;text-align:center}
.tbl tr:hover td{background:#F7FBF7}
.tbl .num{font-weight:650;color:var(--td-xd)}
.tbl td.drill{color:var(--td-d);cursor:pointer;font-weight:650;
 text-decoration:underline dotted}
.tbl td.drill:hover{color:var(--td)}
.tsearch{width:100%;max-width:280px;padding:6px 10px;margin:0 0 8px;
 border:1px solid #D5DDD5;border-radius:8px;font-size:12px;
 background:var(--mist);color:var(--ink);outline:none}
.tsearch:focus{border-color:var(--td)}
.pill{background:#0B3D2E;color:#fff;border-radius:99px;padding:4px 11px;
 font-size:11px;font-weight:650;align-self:flex-end;margin-bottom:2px;
 display:inline-flex;align-items:center;gap:7px}
.pill b{cursor:pointer;font-weight:900;opacity:.75}
...
const state = {tab:"overview", seg:"ALL", dbt:"ALL", own:"ALL",
               sty:"ALL", ao:"ALL", ind:"ALL",
               lstat:"ALL", lsys:"ALL", dbn:"ALL", q:""};
const TABS = [
...
function filtered(){
  ...
  return COMBOS.filter(r =>
    ...
    pass(r,"lstat",state.lstat) && pass(r,"lsys",state.lsys) &&
    pass(r,"dbn",state.dbn) &&
    (!q ||
...
function donut(pairs, unitLabel, clickFn, size){
  ...
  const SZ = size||210, R = SZ*0.343, SW = SZ*0.143, CIRC = 2*Math.PI*R;
  let off = CIRC*0.25;
  ...
  pairs.forEach((p,i)=>{
    ...
    const click = clickFn && p[0]!=="Other" ?
      ` style="cursor:pointer" onclick="${clickFn}('${String(p[0])
        .replace(/'/g,"\\'")}')"` : "";
    s += `<circle cx="${SZ/2}" cy="${SZ/2}" r="${R}" fill="none"${click}
      stroke="${PALETTE[i%PALETTE.length]}" stroke-width="${SW}"
      stroke-dasharray="${dash.toFixed(2)} ${(CIRC-dash).toFixed(2)}"
      stroke-dashoffset="${off.toFixed(2)}"/>`;
    off -= dash;
  });
  s += `<text x="${SZ/2}" y="${SZ/2-3}" text-anchor="middle"
    font-size="${Math.round(SZ*0.114)}" font-weight="700" fill="#0B3D2E"
    font-family="Segoe UI,Arial">${fmt(total)}</text>
    <text x="${SZ/2}" y="${SZ/2+SZ*0.081}" text-anchor="middle"
    font-size="${Math.round(SZ*0.05)}" fill="#6b7f6b"
    font-family="Segoe UI,Arial">${unitLabel}</text></svg>`;
...
/* Sortable + searchable table component.
   - Click a header to sort (toggles asc/desc, numeric-aware).
   - Tables with scroll (or opts.search) get a search box.
   - opts.clickCols = {colIndex: "handlerName"} renders drill-down cells.
   Sort/search state persists per tab while filtering. */
let TBL_SEQ = 0, TBL = {};
const TBLSTATE = {};
function table(headers, rows, opts){
  opts = opts||{};
  if(!rows.length) return "<div class='empty'>No data for current filters</div>";
  const tid = "tbl" + (TBL_SEQ++);
  const key = state.tab + ":" + tid;
  if(!TBLSTATE[key]) TBLSTATE[key] = {sc:-1, dir:1, q:""};
  TBL[tid] = {headers, rows, opts, key};
  return `<div id="${tid}">` + tblInner(tid) + `</div>`;
}
function tblInner(tid){
  const {headers, rows, opts, key} = TBL[tid];
  const st = TBLSTATE[key];
  let view = rows;
  if(st.q){
    const q = st.q.toLowerCase();
    view = view.filter(r=>r.some(c=>String(c??"").toLowerCase().includes(q)));
  }
  if(st.sc >= 0 && st.sc < headers.length){
    view = [...view].sort((a,b)=>{
      const x=a[st.sc], y=b[st.sc];
      if(typeof x==="number" && typeof y==="number") return (x-y)*st.dir;
      return String(x??"").localeCompare(String(y??""),
        undefined,{numeric:true})*st.dir;
    });
  }
  const searchBox = (opts.search || opts.scroll) ?
    `<input class="tsearch" placeholder="Search this table…"
      value="${esc(st.q)}" oninput="searchTbl('${tid}',this.value)">` : "";
  const th = headers.map((h,i)=>
    `<th onclick="sortTbl('${tid}',${i})" title="Click to sort">${esc(h)}${
      st.sc===i ? (st.dir>0?" ▲":" ▼") : ""}</th>`).join("");
  const body = view.slice(0, opts.max||15).map(r=>"<tr>"+r.map((v,i)=>{
    const cls = [];
    let click = "";
    const blank = (v==null || v==="" ||
      (typeof v==="number" && isNaN(v)));
    if(typeof v==="number" && !blank) cls.push("num");
    if(opts.flagCol===i) cls.push(v==="Complete" ? "ok" : "gap");
    if(opts.clickCols && opts.clickCols[i] && !blank){
      cls.push("drill");
      const safe = String(v).replace(/\\/g,"\\\\")
        .replace(/'/g,"\\'").replace(/"/g,"&quot;");
      click = ` onclick="${opts.clickCols[i]}('${safe}')"
        title="Click to drill down"`;
    }
    const txt = blank ? "—" : (typeof v==="number" ? fmt(v) : esc(v));
    return `<td class="${cls.join(" ")}"${click}>${txt}</td>`;
  }).join("")+"</tr>").join("");
  const cnt = st.q ? `<div class="hint">${fmt(view.length)} of ${
    fmt(rows.length)} rows match</div>` : "";
  const t = `<table class="tbl"><thead><tr>${th}</tr></thead>
    <tbody>${body}</tbody></table>`;
  return searchBox + (opts.scroll ? `<div class="scrolltbl">${t}</div>` : t)
    + cnt;
}
function sortTbl(tid, i){
  const st = TBLSTATE[TBL[tid].key];
  if(st.sc === i) st.dir *= -1; else { st.sc = i; st.dir = 1; }
  document.getElementById(tid).innerHTML = tblInner(tid);
}
function searchTbl(tid, v){
  TBLSTATE[TBL[tid].key].q = v;
  const el = document.getElementById(tid);
  el.innerHTML = tblInner(tid);
  const inp = el.querySelector(".tsearch");
...
/* Drill-down setters (no dropdown; cleared via pills or Reset). Labels like
   «No System» drill into the missing-value group. */
const _drillVal = v => String(v).startsWith("\u00ab") ? "Not Specified" : v;
function setLstat(v){ state.lstat=_drillVal(v); render(); }
function setLsys(v){ state.lsys=_drillVal(v); render(); }
function setDbn(v){ state.dbn=_drillVal(v); render(); }
function clearDrill(k){ state[k]="ALL"; render(); }
...
function tabTech(rows){
  ...
  <div class="sec"><h2>Database Inventory (Name × Type)</h2>
    ...
    ${table(["Database Type","Database Name","Outputs","Sources","Rows"],
      inv, {scroll:true,max:500,
      clickCols:{0:"setDbt",1:"setDbn"}})}</div>
  <div class="sec"><h2>Schemas per Database</h2>
    ...
    ${table(["Database Name","Schemas","Elements","Outputs","Rows"],
      ...
      {scroll:true,max:300,clickCols:{0:"setDbn"}})}</div>
...
function tabLineage(rows){
  ...
  <div class="grid2">
    <div class="sec"><h2>Lineage Population Status</h2>
      ...
      Partial = some; No Lineage = none. Click a slice or legend item to
      drill the whole dashboard into that group.</div>
      ${donut([["Full Lineage",full],["Partial Lineage",part],
        ["No Lineage",none]],"rows","setLstat",300)}</div>
    <div class="sec"><h2>Rows by Lineage System</h2>
      <div class="note">Where lineage says the data lands (System field on
      the Data Flow Linkage side). Click a bar to drill down.</div>
      ${hbar(sys,{click:"setLsys"})}</div>
  </div>
  <div class="sec"><h2>Metadata Harvest Priority</h2>
    ...
    ${table(["Source Database","Elements","CDEs","Elements w/o Lineage",
      ...
      harvest, {scroll:true,max:300,search:true,
      clickCols:{0:"setDbn"}})}</div>
  <div class="sec"><h2>Source Database → Lineage System Map</h2>
    ...
    ${table(["Lineage System","Source Database","Elements","Rows"],
      cross, {scroll:true,max:300,search:true,
      clickCols:{0:"setLsys",1:"setDbn"}})}</div>`;
}
...
function render(){
  TBL_SEQ = 0; TBL = {};
  const rows = filtered();
  const active = ["seg","dbt","own","sty","ao","ind","lstat","lsys","dbn"]
    .filter(k=>state[k]!=="ALL").length + (state.q?1:0);
  document.getElementById("f_pills").innerHTML =
    [["lstat","Lineage"],["lsys","Lineage System"],["dbn","Database"]]
    .filter(([k])=>state[k]!=="ALL")
    .map(([k,l])=>`<span class="pill">${l}: ${esc(state[k])}
      <b onclick="clearDrill('${k}')">✕</b></span>`).join(" ");
...
window.addEventListener("DOMContentLoaded", ()=>{
  ...
  document.getElementById("f_reset").addEventListener("click", ()=>{
    Object.assign(state,{seg:"ALL",dbt:"ALL",own:"ALL",sty:"ALL",
      ao:"ALL",ind:"ALL",lstat:"ALL",lsys:"ALL",dbn:"ALL",q:""});
    syncSelects(); render();
  });
  buildNav(); render();
});

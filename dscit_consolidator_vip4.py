...
def dedupe_files(files, keep_latest_per_id=True):
    """Return (files_to_process, skipped) where skipped is a list of
    (path, reason). Two checks:
      1. Identical content (MD5) -> keep first, skip exact copies.
      2. Same DSCIT ID -> keep the newest file, skip superseded ones
         (only when keep_latest_per_id is True)."""
    skipped = []
...
def run_consolidation(folder, progress_cb=None, workers=8,
    ...
    if rows_before:
        combined = combined.drop_duplicates(
            subset=TARGET_COLUMNS, keep="first").reset_index(drop=True)
    dedup = {
        "files_found": total,
...
def _dash_payload(combined: pd.DataFrame):
    ...
    Rows are grouped into weighted combinations of the 12 dimension fields
    ...
    This keeps the HTML small and the in-browser analytics fast even at
    1M+ source rows, while preserving exact distinct counts and filters.
    """
    fields = ["Output Name", "Output Owner", "Source Name", "Source Type",
    ...
    tables = [[] for _ in fields]
    elem_map, elems = {}, []
    combos = {}

    df = combined.reindex(columns=fields + ["Data Element"])
...
function render(){
  const rows = filtered();
  const active = [["seg","seg"],["dbt","dbt"],["own","own"],["sty","sty"],
    ["ao","ao"]].filter(([k])=>state[k]!=="ALL").length + (state.q?1:0);
...
def launch_ui():
    ...
    state = {"running": False, "folder": tk.StringVar(value=""),
             ...
             "keep_latest": tk.BooleanVar(value=True)}
    ...
    dedup_chk = tk.Checkbutton(
        frow, text="De-duplicate: skip identical files, keep only the "
        "latest file per DSCIT ID, and remove duplicate rows",
    ...
    dedup_chk.pack(anchor="w", pady=(8, 0))

...
def write_output(folder, results, combined, analytics=None) -> str:
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as xw:
        ...
        if analytics:
            ...
            analytics["db_type"].to_excel(xw, sheet_name="By Database Type", index=False)
            analytics["db_inventory"].to_excel(xw, sheet_name="DB Inventory", index=False)
            analytics["segment"].to_excel(xw, sheet_name="By Business Segment", index=False)
            analytics["asset_owner"].to_excel(xw, sheet_name="By Asset Owner", index=False)
            analytics["dbs_by_output"].to_excel(xw, sheet_name="DBs by Output", index=False)
            analytics["dbs_by_owner"].to_excel(xw, sheet_name="DBs by Output Owner",
                                               index=False)
            analytics["dbs_by_srctype"].to_excel(xw, sheet_name="DBs by Source Type",
                                                 index=False)
            analytics["source_inventory"].to_excel(xw, sheet_name="Source Inventory & Gaps",
                                                   index=False)
            analytics["dq"].to_excel(xw, sheet_name="Data Quality", index=False)
            if "norm_map" in analytics:
                analytics["norm_map"].to_excel(xw, sheet_name="DB Type Mapping",
                                               index=False)
